...
class App(KeyEditorMixin, PauseEditorMixin, ClickEditorMixin,
    ...
    def __init__(self, root):
        ...
        # New settings ΓÇö defaults loaded from SettingsManager
    ...
    def _btn(self, parent, text, cmd, bg, fg="white", bold=False, state="normal", hover_fg=None, disabledforeground=None, font=None, icon=None, icon_size=14, padx=8, pady=3):
        ...
        b._original_fg = fg
        # Hover changes text color ΓÇö always a visible contrast switch
    ...
    def build_ui(self):
        ...
        self.root.title(f"MacroForge  v{VERSION}  ΓÇö  Macro Automation")
        ...
        # ΓöÇΓöÇ Menu button: gear + dropdown chevron ΓöÇΓöÇ
        ...
        arrow_lbl = tk.Label(menu_wrap, text="Γû╝", bg=C["bg_secondary"], fg=C["text_dim"],
                             font=("Segoe UI", 7))
        ...
        self.status_dot = tk.Label(tb_right, text="ΓÇó", bg=C["bg_secondary"],
        ...
        # Γ₧ò New profile button pinned to the right
        ...
        # ΓöÇΓöÇ LEFT SIDEBAR ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # Add buttons ΓÇö compact sidebar style
        ...
        # ΓöÇΓöÇ Playback config in sidebar ΓöÇΓöÇ
        ...
        for text, var in [
            ("Γê₧ Infinite", self.infinite_loop_var),
            ("Γƒ│ Simulate", self.simulation_var),
            ("Γëê Human",    self.human_curve_var),
            ("ΓèÖ FocusLock",self.focus_lock_var),
        ]:
        ...
        # ΓöÇΓöÇ Recorder section in sidebar ΓöÇΓöÇ
        tk.Frame(sidebar, height=1, bg=C["border"]).pack(fill="x", padx=8, pady=6)
        ...
        self._recorder["status_dot"] = tk.Label(rstatus, text="ΓùÅ", bg=C["bg_tertiary"],
        ...
        # ΓöÇΓöÇ TIMELINE SECTION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ INSPECTOR SECTION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        self.insp_hint = tk.Label(insp, text="ΓåÉ Select to edit",
            bg=C["bg_secondary"], fg=C["border"], font=("Segoe UI", 7, "italic"))
        self.insp_hint.pack(side="left", padx=6)

        # ΓöÇΓöÇ Helper: 4-button row for every inspector ΓöÇΓöÇ
        def _insp4(parent):
        ...
        tk.Label(self.insp_key, text="├ù", bg=C["bg_secondary"], fg=C["text_dim"],
                 font=("Segoe UI", 7)).pack(side="left", padx=(4,1))
        ...
        tk.Label(self.insp_click, text="├ù", bg=C["bg_secondary"], fg=C["text_dim"],
                 font=("Segoe UI", 7)).pack(side="left", padx=(4,1))
        ...
        # ΓöÇΓöÇ PLAYBACK DOCK ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # Transport buttons ΓÇö left of dock
        ...
        # ΓöÇΓöÇ BOTTOM STATUS BAR ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Icon stats (bolt=actions, loop=loops, clock=seq, clock=time) ΓöÇΓöÇ
    ...
    def load_last_session(self):
        ...
        if session:
            try:
                ...
                self.status(f"Profile ΓÇÿ{self.session_manager.active}ΓÇÖ loaded")
            except Exception:
    ...
    def save_session(self):
        """Debounced auto-save ΓÇö coalesces rapid calls into one write 500 ms after last call."""
    ...
    def _make_dialog(self, title, width=420, height=None):
        ...
        if height:
            ...
            dlg.deiconify()               # show now ΓÇö size is known, no resize flicker
        else:
            ...
            def _fit():
                ...
                dlg.resizable(False, False)
                dlg.deiconify()           # show at final size ΓÇö no white flash, no resize flicker
            dlg.after_idle(_fit)
        # Title bar stripe
    ...
    def _dlg_field(self, parent, label, widget_factory, hint=None):
        """Labelled field row. widget_factory(row) ΓåÆ widget."""
        C = self.config.colors
        row = tk.Frame(parent, bg=parent["bg"])
    ...
    # ΓöÇΓöÇ Add Key dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    # ΓöÇΓöÇ Add Delay dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    # ΓöÇΓöÇ Add Click / Edit Click dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    def _toggle_record(self):
        """Start or stop real-time recording.  Never pauses ΓÇö use _toggle_record_pause for that."""
        rec = self._recorder
        if not rec["running"]:
    ...
    def _toggle_record_pause(self):
        ...
        if rec["paused"]:
            ...
            self.status("Recording paused ΓÇö click Resume to continue, or Stop to finish")
    ...
    def apply_changes(self):
        """Deprecated ΓÇô use _apply_inspector. Kept for any external callers."""
    ...
    def _show_action_menu(self, widget):
        ...
        # ΓöÇΓöÇ PROFILES ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        profiles_sub = _menu()
        ...
        _add(profiles_sub, "plus",   "New profileΓÇª", self._new_profile_dialog, foreground=C["accent"])
        _add(profiles_sub, "pencil", "RenameΓÇª",      self._rename_profile_dialog)
        ...
        # ΓöÇΓöÇ ACTIONS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        _add(m, "save",    "Save     Ctrl+S",
             lambda: (self._do_save_session(),
                      self.status(f"Profile ΓÇÿ{self.session_manager.active}ΓÇÖ saved")))
        _add(m, "export",  "Export JSONΓÇª",           self.save)
        _add(m, "import",  "Import JSONΓÇª",           self.load)
        _add(m, "export",  "Export CSVΓÇª",            self.export_csv)
        _add(m, "import",  "Import CSVΓÇª",            self.import_csv)
        m.add_separator()
    ...
    # ΓöÇΓöÇ Update checking ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def _check_update_silent(self):
        """Background update check on startup ΓÇö silent if no update or no URL."""
        def _bg():
    ...
    def _check_update_manual(self):
        ...
        self.status("Checking for updatesΓÇª")
    ...
    def _prompt_update(self, manifest):
        ...
        tk.Label(dlg, text=f"Downloading MacroForge {remote_ver}ΓÇª", bg=C["bg_secondary"],
        ...
        info_lbl = tk.Label(dlg, text="StartingΓÇª", bg=C["bg_secondary"],
        ...
        def _download():
            try:
                if perform_update(manifest, progress_cb=_on_progress):
                    ...
                    self.root.after(0, lambda: (
                        dlg.destroy(),
                        self.status("Update failed ΓÇö check debug log")
                    ))
            except Exception as e:
    ...
    # ΓöÇΓöÇ Profile management helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def _switch_profile(self, name: str):
        ...
        self.update_statistics(immediate=True)
        self._update_profile_tabs()
        self.status(f"Switched to profile ΓÇÿ{name}ΓÇÖ")
        self._update_title()
    ...
    def _new_profile_dialog(self):
        ...
        def on_ok():
            ...
            if not self.session_manager.new_profile(name):
                messagebox.showerror("Exists", f"Profile ΓÇÿ{name}ΓÇÖ already exists.", parent=dlg)
                return
            dlg.destroy()
    ...
    def _rename_profile_dialog(self):
        ...
        sec = self._dlg_section(dlg, f"RENAME ΓÇÿ{old}ΓÇÖ", C["accent"])
        ...
        def on_ok():
            ...
            if not self.session_manager.rename_profile(old, new):
                messagebox.showerror("Failed", f"Could not rename ΓÇö ΓÇÿ{new}ΓÇÖ may already exist.", parent=dlg)
                return
            dlg.destroy()
            self._update_profile_tabs()
            self._update_title()
            self.status(f"Renamed to ΓÇÿ{new}ΓÇÖ")
    ...
    # -*- coding: utf-8 -*-
    r"""Standalone Macro ΓÇö generated by MacroForge"""
    ...
    # ΓöÇΓöÇ SCAN-CODE TABLE (0-9 digits) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    _SC_MAP = {
    ...
    class _Input:
        ...
        def find_image(self, tpl_path, region=None, confidence=0.8):
            """Image search ΓÇö falls back to None if Pillow/pyautogui missing."""
            try:
    ...
    def _run(actions, loops, speed):
        ...
        print("=" * 50)
        print(f"  Actions: {total}  |  Loops: {'Γê₧' if loops < 0 else loops}  |  Speed: {speed}x")
    ...
    '''
                ...
                self.status(f"Compiled standalone ΓåÆ {path} + {bat_path}")
                messagebox.showinfo(
                    "Compiled",
            ...
            try:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        self.status(f"Test compile OK ΓÇö {result.stdout.strip()}")
    ...
    def _update_title(self):
        self.root.title(f"MacroForge  v{VERSION}  ΓÇö  {self.session_manager.active}")
    ...
    def _update_profile_tabs(self):
        ...
        for name in self.session_manager.list_profiles():
            ...
            # Close (├ù) button ΓÇö only for non-default profiles
            if name != ProfileManager.DEFAULT_PROFILE:
                close_btn = tk.Label(inner, text=" ├ù", bg=bg,
                                     fg=C["text_dim"], font=("Segoe UI", 8),
                                     cursor="hand2")
    ...
    def _on_escape(self, event=None):
        ...
        # Running ΓåÆ instant pause + auto-resume countdown
        if self.engine.running and not self.engine.paused:
            self.engine.toggle_pause()
            self._start_resume_countdown(3)
            return

        # Paused with no countdown ΓåÆ resume immediately
        if self.engine.running and self.engine.paused:
            self.engine.toggle_pause()
            return

        # Nothing running ΓÇö silent ESC, give brief feedback
        if not self.engine.running:
            self.status("Nothing playing  ΓÇö  press Start to begin")
    ...
    def start(self):
        # Guard: already running (not paused) ΓÇö ignore duplicate
        if self.engine.running and not self.engine.paused:
            return
        ...
        try:
            ...
            if self.engine.focus_lock:
                ...
                self.status("Focus locked ΓÇö will restore window before each action")
    ...
    def _on_key_press_global(self, key):
        """Global hotkeys ΓÇö dispatched on main thread via after().
